...
const contractAddress = "YOUR_CONTRACT_ADDRESS"; // Replace with Remix-deployed address
const contractABI = [/* ABI from Remix */]; // Replace with ABI from Remix
...
async function generatePrescription(event) {
    ...
    try {
        ...
        await contract.methods.addPrescription(name, age, gender, disease).send({ from: accounts[0] });

        document.getElementById("prescription").innerHTML = `
            <h3>Prescription Stored on Blockchain</h3>
        ...
        console.error(error);
        alert("Failed to store prescription on blockchain.");
    }
}
